...
ParserArgnameArgvalToCtype::errcode_t parser_charstr_to_long_int(
	...
	if (input->data_type == ParserArgnameArgvalToCtype::DataType::CHARSTR) {
		if (output->data_type == ParserArgnameArgvalToCtype::DataType::LONG) {
			/*================*/
			/* parsing itself */
			char* parse_remainder = nullptr;
			*(long*) output->memory = strtol((char*) input->memory, &parse_remainder, 10);
			output->memory_bytelen = (int)sizeof(long);
			/* end of parsing */
			/*================*/
		} else {
			retval =
