...
ParserArgnameArgvalToCtype::errcode_t parser_charstr_to_charstr_path(
	...
	ParserArgnameArgvalToCtype::errcode_t retval =
			ParserArgnameArgvalToCtype::RETVAL_OK;
	...
	if (input->data_type == ParserArgnameArgvalToCtype::DataType::CHARSTR) {
		if (output->data_type
			...
			if ((input->memory_bytelen > 0)
					&& (input->memory_bytelen < output_len))
				output_len = input->memory_bytelen;

			for (unsigned int i = 0; i < output_len; i++) {
				/* SANITIZE INPUT HERE */
				if (((char*) input->memory)[i] == 'i'/*0*/) { //filter out undesirable characters
					((char*) output->memory)[i] = 'I'; //replaced with something else or do other sanitation
				} else {
		...
		retval =
				ParserArgnameArgvalToCtype::RETVAL_ERROR_INPUT_TYPE_UNSUPPORTED_BY_PARSER_FUNC;
		goto end;
	}
	end: return retval;
}
